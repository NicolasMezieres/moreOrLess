function verificationAnswer(answer, start, min = 1, max = 100) {
  if (Number.isInteger(Number(answer)) && ((answer >= min && answer <= max) || start)) {
    return true;
  } else {
    console.log(Number.isInteger(answer) && ((answer >= min && answer <= max) || start));
...
while (play) {
  ...
  while (rangeSetting) {
    ...
    console.log(verificationAnswer(lowerTemp, true) && verificationAnswer(higherTemp, true));
  ...
  while (game) {
    ...
    if (correctAnswer) {
      loop++;
      moreOrLessMessage(nbrRandom, nbrAnswer);
      if (triesNbr == loop) {
